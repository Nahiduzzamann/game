...
import { uploadImageBanner, uploadImageSquire } from "./fileUploadController.js";
import { BonusHistory, Deposit, History, PromotionHistory, Promotions, UserWallets, Users, Wallets, Withdraws, } from "../connections/databaseConnection.js";
import { ObjectId } from "mongodb";
import { DATATYPES, sendNotificationToAdmin, sendNotificationToUser } from "../functions/sendNotification.js";
...
export const createWallet = async (req, res) => {
    ...
    try {
        const { path } = (await uploadImageSquire(req, res));
...
export const updateWallet = async (req, res) => {
    ...
    try {
        const { path } = req.file ? (await uploadImageSquire(req, res)) : { path: undefined };
...
export const deleteUserWallets = async (req, res) => {
    const { id } = req.params;
    try {
...
export const createWithdraw = async (req, res) => {
    ...
    try {
        ...
        const promotion = deposit[0].promotionId != "undefined" && deposit[0].promotionId
    ...
    try {
        ...
        await Promise.all(runningTurn.map(async (turn, i) => {
            if (!turn.completed && turn.promotion) {
                const gameHistory = await History.find({ username: username, date: { $gte: turn.date } });
                ...
                if (totalTurn > turn.promotion.turnOverAmount) {
                    ...
                    const deposit = await Deposit.find({ userId: username, promotionId: turn.promotionId }).sort({ date: -1 });
